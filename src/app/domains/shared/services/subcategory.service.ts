...
export class SubcategoryService {
  ...
  getAll() {
    return this.http.get<Subcategory[]>('http://localhost:3000/api/v1/subcategories');
  }

  getOne(id: string | number) {
    return this.http.get<Subcategory>(`http://localhost:3000/api/v1/subcategories/${id}`);
  }

  // /subcategories/:id/products (si lo tienes en tu API)
  getProductsBySubcategory(id: string | number) {
    return this.http.get<Product[]>(`http://localhost:3000/api/v1/subcategories/${id}/products`);
  }
