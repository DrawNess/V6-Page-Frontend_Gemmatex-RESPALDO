...
type NavSection = { title: string; items: NavItem[]; svgPath: string; visibleFor: string[] };
...
export class AdminSidebarComponent {
  ...
  private branchCache = inject(BranchCacheService);
  ...
  readonly adminRoles = this.tokenService.getRolesFromToken();
  readonly adminRole  = this.adminRoles.join(', ') || 'admin';
  ...
  // `staff` global del SSO cubre los roles fine-grained de sucursal
  // (branch_admin/seller/cashier/etc.) que viven en `user_branches` del API-V6.
  readonly hasBranchScope = !this.isGlobalAdmin && this.tokenService.hasRole('staff');

  readonly branchScopeCities: string = (() => {
    if (this.isGlobalAdmin) return 'Todas las sucursales';
    const ids = this.tokenService.getBranchesFromToken();
    const branches = this.branchCache.resolveBranchIds(ids);
    const cities = [...new Set(branches.map(b => b.city))];
    return cities.length > 0 ? cities.join(', ') : '—';
  })();

  get visibleNav(): NavSection[] {
    const roles = this.tokenService.getRolesFromToken();
    return this.nav.filter(s => s.visibleFor.some(r => roles.includes(r)));
  }
  ...
  readonly nav: NavSection[] = [
    {
      ...
      visibleFor: ['admin', 'super_admin', 'staff', 'branch_admin'],
      items: [
    ...
    },
    {
      ...
      visibleFor: ['admin', 'super_admin', 'staff', 'branch_admin', 'seller'],
      items: [
